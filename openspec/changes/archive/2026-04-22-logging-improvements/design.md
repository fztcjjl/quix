...
**选择**：在 `quix.New()` 中，当 `env == EnvDev` 时，通过 `log.NewZerolog(l, log.WithCaller())` 启用 caller 字段。不使用 zerolog 的 `CallerWithSkipFrameCount` hook 机制，而是在 zerologLogger 适配器内部通过 `findCaller()` 遍历调用栈（`runtime.CallersFrames`），跳过 `runtime.`、`testing.`、`/core/log/` 内部帧，返回第一个用户代码帧。生产环境不启用。

**理由**：zerolog 的 `CallerWithSkipFrameCount` 配置固定 skip count，无法同时适配 `log.Info()`（2 层包装帧）和 `log.FromContext(ctx).Info()`（1 层包装帧）两种调用路径。栈帧遍历方式对任意调用深度都正确，且仅开发环境启用，性能损耗（最多 3-4 次 `runtime.Caller`）可接受。`With()` 创建的 child logger 自动继承 `callerEnabled`，确保 WithRequestLogger 注入的 context logger 也正确输出 caller。

**替代方案**：
- 固定 `CallerWithSkipFrameCount(N)` — 任何固定值都无法同时满足不同调用路径
- CtxXxx 便捷函数 — 不修复 `FromContext().Xxx()` 直接调用的 caller 偏移问题，且增加 5 个公开 API
